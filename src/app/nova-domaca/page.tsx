    ...
    <PageShell>
      <main className="bg-white">
        ...
        <section className="bg-[#F7F5FC] px-5 py-24 md:px-8 md:py-32">
          <div className="mx-auto grid max-w-[1080px] items-center gap-14 lg:grid-cols-[.9fr_1.1fr]">
            <div><Kicker>Jasna cena</Kicker><h2 className="text-balance text-[36px] font-extrabold leading-[1.12] tracking-[-.035em] md:text-[48px]">Vse potrebno za profesionalen začetek in dolgoročno rast.</h2><p className="mt-5 text-[16px] leading-[1.7] text-[#66616E]">Brez visokega začetnega vložka, brez tehničnih skrbi in brez vezave. Če potrebujete dodatno pomoč, so Google Business Profil, SEO in oglaševanje na voljo posebej.</p>
            ...
            <div className="rounded-[28px] border border-[#DED9F2] bg-white p-7 shadow-[0_25px_70px_rgba(57,43,113,.13)] sm:p-10"><div className="flex items-center justify-between gap-4"><div><div className="text-sm font-extrabold text-[#6654DB]">Obrtio osnovni paket</div><div className="mt-3 flex items-end gap-2"><span className="text-[52px] font-extrabold leading-none tracking-[-.05em]">99 €</span><span className="pb-1 text-sm text-[#787380]">/ mesec</span></div></div><span className="rounded-full bg-[#EEEAFE] px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-wider text-[#6654DB]">Vse vključeno</span></div><div className="my-8 h-px bg-[#ECEAF1]" /><div className="grid gap-3 sm:grid-cols-2">{included.map((item) => <div key={item} className="flex gap-2.5 text-[13px] font-semibold leading-5 text-[#4E4A55]"><span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-[#E9F8F1] text-[#16865D]"><Check size={12} strokeWidth={3} /></span>{item}</div>)}</div><Button asChild size="lg" className="mt-9 w-full"><Link href="/kontakt">Dogovorite se za brezplačen posvet <ArrowRight size={17} /></Link></Button><p className="mt-4 text-center text-[11px] text-[#8E8996]">Ali 950 € za 12 mesecev · prekinitev kadarkoli</p></div>
          </div>
        </section>
