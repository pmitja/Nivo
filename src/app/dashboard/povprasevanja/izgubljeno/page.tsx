...
export default async function LostLeadsPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  const user = await requireClientUser();
  const params = await searchParams;
  const page = Number(params.page ?? "1");
  const [data, sentReviewLeadIds] = await Promise.all([
    getClientLeadsPage(user.companyId!, "lost", Number.isFinite(page) ? page : 1),
    ...
    <DashboardShell user={user} mode="client" title="Lost" subtitle="Inquiries that did not turn into booked work.">
      <Panel title="Lost">
        ...
        <PaginationFooter
          ...
          total={data.total}
          basePath="/dashboard/povprasevanja/izgubljeno"
        />
      </Panel>
    </DashboardShell>
  );
