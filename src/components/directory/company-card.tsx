...
export function CompanyCard({ company, locale = "sl" }: { company: DirectoryCompany; locale?: "sl" | "en" }) {
    ...
    <article className="group flex flex-col rounded-[20px] border border-[#ECEAF3] bg-white p-6 shadow-[0_2px_10px_rgba(20,19,29,.04)] transition-all duration-200 hover:-translate-y-1 hover:border-[#D8D2F0] hover:shadow-[0_18px_44px_rgba(20,19,29,.10)]">
      <div className="flex items-start gap-4">
        {company.logoUrl ? (
          <img
            src={company.logoUrl}
            alt={english ? `${company.name} logo` : `Logotip podjetja ${company.name}`}
            width={52}
            height={52}
      ...
      <div className="mt-4 flex flex-col gap-2 text-[14px] text-[#54515E]">
        <div className="flex items-center gap-2">
          ...
          {company.city} {english ? "and nearby" : "in okolica"}
        </div>
      ...
      <div className="mt-5 flex flex-wrap gap-2.5 border-t border-[#F1EFF7] pt-5">
        <Button asChild size="sm" className="flex-1">
          <Link href={companyContactUrl(company)}>{english ? "Send inquiry" : "Pošlji povpraševanje"}</Link>
        </Button>
        {website ? (
          <Button asChild size="sm" variant="secondary" className="flex-1">
            <a href={website} target="_blank" rel="noopener noreferrer">
              {english ? "Website" : "Spletna stran"} <ArrowUpRight size={15} />
            </a>
          </Button>
        ) : null}
      </div>
