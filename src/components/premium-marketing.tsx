...
export function MarketingHero({ eyebrow, title, highlight, text, secondary, children }: { eyebrow: string; title: string; highlight: string; text: string; secondary?: { label: string; href: string }; children?: React.ReactNode }) {
  return <section className="relative isolate overflow-hidden bg-[#F7F5FC] px-5 pb-20 pt-20 text-center md:px-8 md:pb-28 md:pt-28">
    ...
    <div className="nv-enter mx-auto max-w-[880px]">
      ...
      <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row"><Button asChild size="lg"><Link href="/kontakt">Brezplačen posvet <ArrowRight size={17} /></Link></Button>{secondary ? <Button asChild size="lg" variant="secondary"><Link href={secondary.href}>{secondary.label}</Link></Button> : null}</div>
      <div className="mt-5 flex flex-wrap justify-center gap-x-5 gap-y-2 text-[12px] font-semibold text-[#7B7686]">{["Brez vezave", "Vse uredimo mi", "Od 99 € / mesec"].map(x => <span key={x} className="flex items-center gap-1.5"><Check size={14} className="text-[#219A6B]" />{x}</span>)}</div>
    </div>
