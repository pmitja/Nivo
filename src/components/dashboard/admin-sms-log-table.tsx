...
import { formatCurrency, formatDate, smsStatusLabels, smsTypeLabels } from "@/lib/labels";
import type { getAdminSmsPage } from "@/lib/dashboard-data";

type AdminSmsPage = Awaited<ReturnType<typeof getAdminSmsPage>>;

export function AdminSmsLogTable({ data }: { data: AdminSmsPage }) {
  ...
  return (
    <div className="overflow-hidden rounded-[16px] border border-[#EEEAF5]">
      <div className="hidden grid-cols-[1fr_.8fr_1.25fr_.7fr_auto] gap-3 border-b border-[#EEEAF5] bg-[#FBFAFF] px-4 py-3 text-xs font-extrabold uppercase tracking-[.06em] text-[#8D8999] lg:grid">
        <div>Stranka</div>
        <div>Telefon</div>
        <div>Sporočilo</div>
        <div>Provider</div>
      ...
      <div className="divide-y divide-[#EEEAF5] bg-white">
        {data.messages.map((sms) => (
          <div key={sms.id} className="grid gap-2 px-4 py-3 lg:grid-cols-[1fr_.8fr_1.25fr_.7fr_auto] lg:items-center">
            <div>
              <div className="text-sm font-extrabold">{sms.companyName}</div>
              <div className="mt-0.5 text-xs font-semibold text-[#8A8694]">
                {smsTypeLabels[sms.type]} · {formatDate(sms.createdAt)}
              </div>
            </div>
            ...
            <div className="text-xs font-bold text-[#827E8D]">
              {sms.provider}
              {sms.cost ? ` · ${formatCurrency(Number(sms.cost))}` : ""}
            </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[#EEEAF5] bg-[#FBFAFF] px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          <Button asChild variant="secondary" size="sm" disabled={data.page <= 1}>
            <Link href={`/admin/sms-log?smsPage=${Math.max(1, data.page - 1)}`}>Nazaj</Link>
          </Button>
          ...
          <Button asChild variant="secondary" size="sm" disabled={data.page >= data.pageCount}>
            <Link href={`/admin/sms-log?smsPage=${Math.min(data.pageCount, data.page + 1)}`}>Naprej</Link>
          </Button>
        </div>
      </div>
    </div>
